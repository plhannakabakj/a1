...
def start_attack_reply(message, target, port, time):
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("🍷ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ🍷", url="https://t.me/NoxxNetwork")
    )
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi1_cooldown and (datetime.datetime.now() - bgmi1_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait Before Running The /bgmi1 Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐩𝐫𝐨𝐯𝐢𝐝𝐞🚀: /bgmi1  <𝐇𝐨𝐬𝐭> <𝐏𝐨𝐫𝐭> <𝐓𝐢𝐦𝐞>"  # Updated command syntax 
    else:
